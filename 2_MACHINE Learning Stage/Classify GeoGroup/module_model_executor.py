...
class ModelExecutor:
    ...
    def execute_single_training_run(self, method, X_train, y_train, X_test, y_test, target):
        ...
        try:
            ...
            for name, metric in self.metrics.items():
                scores[name] = metric(y_test, y_class)
    ...
    def execute(self, df, cols_train, cols_target=None, folder_suffix=None, selected_methods=None):
        ...
        if self.use_ensemble:
            ...
            y_class = ensemble.predict(X_test)
            ensemble_scores = self.calculate_scores(y_test, y_class)
    ...
    def calculate_scores(self, y_test, y_class):
        scores = {}
        for name, metric in self.metrics.items():
            scores[name] = metric(y_test, y_class)
        return scores
